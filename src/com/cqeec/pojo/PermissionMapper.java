...
public class PermissionMapper {
  ...
  public void delete(Long primaryKey) {
    String sql=SqlUtil.getDeleteSql(Permissions.class)+"where "+ClassUtil.getPrimaryKeyByClass(Permissions.class)+"=?";
    SqlUtil.delete(sql,primaryKey);
  }

  public void update(Permissions permission) {
    String sql=SqlUtil.getUpdateSql(permission.getClass())+"where "+ClassUtil.getPrimaryKeyByClass(Permissions.class)+"=?";
    SqlUtil.modify(sql,CollectionUtil.sortByUpdate(permission));
  }

  public Permissions select(Long primaryKey) {
    String sql=SqlUtil.getSelectSql(Permissions.class, "where "+ClassUtil.getPrimaryKeyByClass(Permissions.class)+"=?");
    List<Object> temp=SqlUtil.select(sql,Permissions.class,primaryKey);
    return temp!=null&&temp.size()!=0?(Permissions)temp.get(0):null;
  }

  public void batchInsert(List<Permissions> permissions) {
  ...
  public void batchUpdate(List<Permissions> permissions) {
    for(Permissions permission:permissions) {
      update(permission);
    }
  }

  public List<Permissions> batchSelect(Long[] primaryKeys) {
  ...
  public List<Permissions> batchSelect(List<Long> primaryKeys) {
    List<Permissions> permissions=new ArrayList<>();
    for(Long primaryKey:primaryKeys) {
      permissions.add(select(primaryKey));
    }
  ...
  public void deleteByCondition(Condition condition) {
    List<Permissions> list=selectByCondition(condition);
    for(Permissions permission:list)  {
      delete((Long)ColumnUtil.callPKGetMethod(permission));
    }
  }

  public List<Permissions> selectByCondition(Condition condition) {
    String sql=SqlUtil.getSelectSql(Permissions.class, condition!=null?condition.generateCondition():null);
    List<Permissions> objs=new ArrayList<>();
    List<Object> list=SqlUtil.select(sql, Permissions.class,condition!=null?condition.generateParams():null);
    for(Object object:list) {
      objs.add((Permissions)object);
    }
    return objs;
  }

  public PageInfo<Permissions> selectByConditionWithPagination(Condition condition,
  ...
  @Deprecated
  public void insertBySql(String sql, Permissions permission) {
    SqlUtil.save(sql, permission);
  }
  ...
  public void deleteBySql(String sql_, Object[] params) {
    ...
    String sql=SqlUtil.getSelectSql(Permissions.class, null);
    sql+=" where "+arrStr[1];
    List<Object> list=SqlUtil.select(sql,Permissions.class, params);
    for(Object object:list) {
      SqlUtil.delete(SqlUtil.getDeleteSql(Permissions.class)+"where "+ClassUtil.getPrimaryKeyByClass(Permissions.class)+"=?", (Long)ColumnUtil.callPKGetMethod(object));
    }
  }
  ...
  @Deprecated
  public List<Permissions> selectBySql(String sql, Object[] params) {
    List<Permissions> list=new ArrayList<>();
    List<Object> temps=SqlUtil.select(sql, Permissions.class, params);
    for(Object temp:temps) {
      list.add((Permissions)temp);
    }
    return list;
  }
  ...
  public static class Condition {
    ...
    public Condition andUrlNotNull() {
      return simplify("url"+" is not null ",null);
    }

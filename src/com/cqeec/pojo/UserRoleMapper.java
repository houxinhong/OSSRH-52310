...
public class UserRoleMapper {
  ...
  public void insert(UserRole userRole) {
    ...
    Object result=SqlUtil.save(sql,userRole);
    if(result!=null){String pkFieldName=ClassUtil.getPrimaryKeyFieldName(userRole.getClass());
            Field field=FieldUtil.getFieldByColumnName(userRole.getClass(), pkFieldName);
    	    Object id=null;
    	    if(field.getType().equals(Integer.class))id=result;
    	    if(field.getType().equals(Long.class))id=Long.valueOf(result.toString());
            if(id!=null)ClassUtil.invokeSet(userRole, pkFieldName, id); }}
  ...
  public void deleteByCondition(Condition condition) {
    ...
    for(UserRole userRole:list)  {
      delete((Long)ColumnUtil.callPKGetMethod(userRole));
    }
  }
  ...
  public PageInfo<UserRole> selectByConditionWithPagination(Condition condition,
      PageInfo<UserRole> pageInfo) {
    String sql=SqlUtil.getSelectSql(UserRole.class, condition!=null?condition.generateCondition():null);
    List<UserRole> objs=new ArrayList<>();
    List<Object> list=SqlUtil.select(sql, UserRole.class,condition!=null?condition.generateParams():null);
    for(Object object:list) {
  ...
  public void deleteBySql(String sql_, Object[] params) {
    ...
    for(Object object:list) {
      SqlUtil.delete(SqlUtil.getDeleteSql(UserRole.class)+"where "+ClassUtil.getPrimaryKeyByClass(UserRole.class)+"=?", (Long)ColumnUtil.callPKGetMethod(object));
    }
  }
  ...
  public static class Condition {
    ...
    public Condition andUidNotNull() {
      return simplify("uid"+" is not null ",null);
    }
